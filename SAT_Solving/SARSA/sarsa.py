...
def set_var(literal, boolean, literal_clauseNum, clauseNum_clause, literal_boolen):
    ...
    literal_clauseNum[opposite_literal] = set()  # The opposite literal has been made False and is not watching any clauses anymore
...
def sarsa(env, estimator, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0):
    """
    ...
    while True:
        ...
        td_target = reward + discount_factor * q_values[next_action]
...
def train(training_files, epochs, ϵ, epsilon_decay=0.97):
    ...
    # We get knowledge of other files too when re-iterating over already seen files
        ...
        for j, filepath in enumerate(training_files):
            ...
            env = Env(filepath)
            episode_reward, episode_length, estimator, loss = sarsa(env, estimator, epsilon=ϵ, epsilon_decay=epsilon_decay_j)
...
#     episode_reward_train, episode_length_train, estimator, losses = train(training_files, epochs=1, ϵ=0.8, epsilon_decay=0.92)
    ...
    with open('Qmetrics3.pickle', 'rb') as fin:
        estimator, episode_reward_train_3, episode_length_train_3 = pickle.load(fin)
    
    s = time.time()
    episode_reward_test, episode_length_test, episode_actions, episode_rewards = test(files, ϵ=0, estimator=estimator)
    e = time.time()
    ...
    s = time.time()
    episode_reward_rand, episode_length_rand, episode_actions_rand, episode_rewards_rand = test(files)
    e = time.time()
...
#     with open('metrics3.pickle', 'wb') as fout:
#         pickle.dump((estimator, episode_reward_train, episode_length_train), fout)